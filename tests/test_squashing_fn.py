...
from pilco.controllers import squash_sin, squash_cum_normal, squash_tanh
...
def test_tanh_squash():
    is_correct = squashing_fn(squash_tanh, tanh_squash)
    if is_correct:
        print('tanh_squash passes test')
...
def squashing_fn(
    ...
    if dims > 1:
        ...
        # covariance = tf.linalg.diag(tf.ones(dims, dtype=np.float64))
    ...
    # check_owenT = (sampled_covariance + tf.matmul(M, M) - M) / (- 2)
    ...
    mean_and_covariance_correct = np.allclose(M, sampled_means, atol=.03) and \
                                  ...
                                  np.allclose(V, sampled_input_output_covariance, atol=.03)
...
def tanh_squash(input: np.ndarray) -> np.ndarray:
    return tf.math.tanh(input)

if __name__ == '__main__':
    # unittest.main()
    # test_sin_squash()
    test_cum_normal_squash()
    # test_tanh_squash()