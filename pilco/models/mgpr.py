...
class MGPR(gpflow.Module):
    def __init__(self, data, name=None):
        gpflow.Module.__init__(self, name)
    ...
    def optimize(self, restarts=1):
        ...
        for model, optimizer in zip(self.models, self.optimizers):
            best_params = {
                "lengthscales" : model.kernel.lengthscales._value(),
                "k_variance" : model.kernel.variance._value(),
                "l_variance" : model.likelihood.variance._value()}
            ...
            for restart in range(restarts):
                ...
                if loss < best_loss:
                    best_params["k_lengthscales"] = model.kernel.lengthscales._value()
                    best_params["k_variance"] = model.kernel.variance._value()
                    best_params["l_variance"] = model.likelihood.variance._value()
                    best_loss = model.training_loss()
    ...
    def lengthscales(self):
        return tf.stack(
            [model.kernel.lengthscales._value() for model in self.models]
        )

    @property
    def variance(self):
        return tf.stack(
            [model.kernel.variance._value() for model in self.models]
        )

    @property
    def noise(self):
        return tf.stack(
            [model.likelihood.variance._value() for model in self.models]
        )
